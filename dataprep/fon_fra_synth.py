...
def parse_args():
    ...
    parser.add_argument('--base_path', type=str, default='fon', help='Base path')
...
def main(args):
    ...
    # read the json file
    train_json = os.path.join(args.base_path, 'fon_fra.json')
    train_dict = {}
    with open(train_json, 'r') as f:
        dataset = json.load(f)
        for i, item in enumerate(dataset):
            audio_id = item["audio"]
            transcript = item["fon_transcript"]
            translation = item["fra_translation"]
            train_dict[audio_id] = {
